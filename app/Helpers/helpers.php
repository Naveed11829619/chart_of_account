...
//         $subAccount = FinalAccount::where('id',$subAccountID)->first();
//         $openingBalance = $subAccount->opening_balance;
//         $transactionType = $subAccount->transaction_type;
//         $vouchers = $specific?VoucherDetail::where('id','!=',$id)->where('final_account_id',$subAccount->id)->whereBetween('date',[$startDate, $endDate])->get():VoucherDetail::where('final_account_id',$subAccount->id)->whereBetween('date',[$startDate, $endDate])->get();
//         if($vouchers->count() > 0){
...
if(!function_exists('getOpeningBalance')){
    function getOpeningBalance($subAccountID,$startDate, $endDate, $specific=false,$id=0){

        $subAccount = FinalAccount::where('id',$subAccountID)->first();
        // dd(VoucherDetail::where('final_account_id',$subAccount->id)->get);
        $openingBalance = $subAccount->opening_balance;
        $transactionType = $subAccount->transaction_type;

        if($startDate == null && $endDate != null){
            $vouchers = $specific?VoucherDetail::whereDate('date', $endDate)->get():VoucherDetail::where('final_account_id',$subAccount->id)->whereBetween('date',[$startDate, $endDate])->get();
        }
        if($endDate == null && $startDate != null){
            $vouchers = $specific?VoucherDetail::whereDate('date', $startDate)->get():VoucherDetail::where('final_account_id',$subAccount->id)->whereBetween('date',[$startDate, $endDate])->get();
        }
        if($endDate != null && $startDate != null){
            $vouchers = $specific?VoucherDetail::whereBetween('date',[$startDate, $endDate])->get():VoucherDetail::where('final_account_id',$subAccount->id)->whereBetween('date',[$startDate, $endDate])->get();
        }
...
// dd($startDate,$endDate);
        ...
        return ["opening_balance"=>$openingBalance,"opening_balance_type"=>$transactionType,"start_date"=>$startDate,"end_date"=>$endDate];
    }
}

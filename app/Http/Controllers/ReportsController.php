...
class ReportsController extends Controller
{
    // Party Account Report code Start
      public function partyAccountReport($sub_account_id,$start_date,$end_date){
        ...
        $vouchers = VoucherDetail::where('final_account_id',$sub_account_id)->whereBetween('date',[$start_date, $end_date])->orderBy('date','asc')->get();
        $totalDebit = VoucherDetail::where('final_account_id',$sub_account_id)->whereBetween('date',[$start_date, $end_date])->sum('debit_amount');
        $totalCredit = VoucherDetail::where('final_account_id',$sub_account_id)->whereBetween('date',[$start_date, $end_date])->sum('credit_amount');
        $subAccount = FinalAccount::where('id', $sub_account_id)->first();
        // dd($subAccount);
  ...
  // journal Report code start
    public function journalReport()
    {
        ...
        $dates = Session::get('key');
        $vouchers = VoucherDetail::whereBetween('date',[$dates['start_date'],$dates['end_date']])->get();
    ...
    public function trialReport( $start_date , $end_date){
        ...
        // $subAccounts = FinalAccount::where('level_three_id',$id)->get();
        $subAccounts = Session::get('key');
        $startDate = $start_date;
        $endDate = $end_date;
    ...
    Public function finalAccountAccountReport($start_date,$end_date){
        ini_set ( 'max_execution_time', -1); //unlimit
        $vouchers = VoucherDetail::whereBetween('date',[$start_date, $end_date])->orderBy('date','asc')->get();
        $totalDebit = VoucherDetail::whereBetween('date',[$start_date, $end_date])->sum('debit_amount');
        $totalCredit = VoucherDetail::whereBetween('date',[$start_date, $end_date])->sum('credit_amount');
        $subAccount = FinalAccount::all();
        $sumOfDebit = VoucherDetail::select('debit_amount')->whereBetween('date',[$start_date, $end_date])->sum('debit_amount');
        $sumOfCredit = VoucherDetail::select('credit_amount')->whereBetween('date',[$start_date, $end_date])->sum('credit_amount');
        // dd($subAccount);
        $startDate = $start_date;
        $endDate = $end_date;
        $pdf = PDF::loadView('admin.pdf_reports.final_report', compact('sumOfCredit','sumOfDebit','vouchers','totalDebit','totalCredit','subAccount','endDate','startDate'));
      // download PDF file with download method
        return $pdf->download('Final_'.'ledger'.'.pdf');
    }

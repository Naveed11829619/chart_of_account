    ...
    <tbody>
        ...
        @if(isset($vouchers) && $vouchers->count()>0)
            @foreach($vouchers as $key=>$singleAccount)
                @php
                    $subAccount = App\FinalAccount::where('id',$key)->first();
                    $recordEntryType = '';
                ...
                @foreach($reverseArray as $key=>$detail)
                    @if($loop->first)
                        @php
                            $getOpeningBalanceResponse = getOpeningBalance($detail['final_account_id'],$detail['date'],true,$detail['id']);
                        ...
                        @if ($openingBalance != 0)
                             <tr>
                                ...
                                <td colspan="2"></td>
                                <td>{{ number_format(getOpeningBalance($detail['final_account_id'],$detail['date'],true,$detail['id'])["opening_balance"]) }}</td>
                                <td>{{ getOpeningBalance($detail['final_account_id'],$detail['date'],true,$detail['id'])["opening_balance_type"] }}</td>
                            </tr>
                        @endif
                    @endif
